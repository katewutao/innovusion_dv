...
def rewrite_print(log_path):
    ...
    def print_res(*args, **kwargs):
        ...
        builtins.print_origin(f"[{current_date}] {msg}",
            ...
            if ret:
                ...
                if column_name in spec_df.index:
                    if not pd.isna(spec_df.loc[column_name, "LL"]) and value_float < spec_df.loc[column_name, "LL"]:
                        print(
                            f"{ip} {column_name} is {value_float},below LL {spec_df.loc[column_name,'LL']}")
                        ...
                        print(
                            f"{ip} {column_name} is {value_float},above UL {spec_df.loc[column_name,'UL']}")
                        # new_item.setForeground(QtGui.QColor(255,0,0))
                        new_item.setBackground(QtGui.QColor(255, 0, 0))
...
def init_power():
    ...
    if list_in_str(["FT232", "0403:6001", "067b:23c3"], res):
        shutil.copyfile(os.path.join(os.getcwd(), "power_DH.py"),
                        os.path.join(os.getcwd(), "power.py"))
        return True
    elif list_in_str(["HL-340", "PL2303", "1a86:7523"], res):
        shutil.copyfile(os.path.join(os.getcwd(), "power_PY.py"),
                        os.path.join(os.getcwd(), "power.py"))
        return True
...
def get_time():
    times_now = time.strftime('%Y.%m.%d %H:%M:%S ',
...
class DSP_info_thread(QThread):  # TODO
    def __init__(self, ip, record_folder):
        ...
        # API time interval = socket time interval, so not using API
        self.command = f"http://{ip}"
    ...
    def extract_keys(self, res):
        ...
        for item in self.search_keys.items():
            if item[1] == "":
                re_list.append(datetime.datetime.now().strftime(
                    '%Y-%m-%d %H:%M:%S.%f')[:-3])
            else:
    ...
    def run(self):
        ...
        self.record_file = os.path.join(
            self.save_folder, f"{self.ip.replace('.','_')}.csv")
        if not os.path.exists(self.record_file):
            csv_write(self.record_file, [
                      "timestamp"]+list(self.read_keys.keys()))
        while True:
...
class one_lidar_record_thread(QThread):
    ...
    def run(self):
        ...
        while True:
            sn_res = get_curl_result(
                f"http://{self.ip}:8010/command/?get_sn", 1)
            if sn_res[1]:
                sn = sn_res[0]
                break
        while True:
...
class MonitorFault(QThread):
            ...
            if re.match(key, file):
                ...
                if self.raw_list[file] > self.max_raw:
                    try:
                        ...
                        print(
                            f"{self.ip} {file} count {self.raw_list[file]},remove success")
                    except:
                        print(
                            f"{self.ip} {file} count {self.raw_list[file]},remove failed")
                return True
        return False
    ...
    def get_cmd_print(self, fault_log_path):
        ...
        if ret:
            ...
            with open(fault_log_path, "a") as f:
                f.write(
                    f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}]{str1}\n")
            print(str1)
            ret_fault = re.search("IN_FAULT_([A-Z_0-9]+)", ret.group(2))
            if ret_fault:
                if ret.group(3) == "set":
                    self.sigout_fault_info.emit(
                        ret_fault.group(1), self.row_idx)
                else:
                    self.sigout_fault_heal.emit(
                        ret_fault.group(1), self.row_idx)
    ...
    def reboot_cmd(self, command1, command2, command3, command4):
        ...
        self.cmd = subprocess.Popen(
            command1, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
        time.sleep(1)
        if self.isInterruptionRequested():
            return
    ...
    def run(self):
        ...
        if not os.path.exists(client_log_folder):
            try:
                ...
                pass
        fault_log_path = os.path.join(
            fault_log_path, self.ip.replace(".", "_")+".txt")
        client_log_path = os.path.join(
            client_log_folder, f"{get_current_date()}.txt")
        ...
        while True:
            ...
            self.delete_util_log(os.path.join(
                util_dir, "inno_pc_client.log.err"))
            self.delete_util_log(os.path.join(
                util_dir, "inno_pc_client.log.1"))
            self.delete_util_log(os.path.join(
                util_dir, "inno_pc_client.log.2"))
...
class PointCloud(QThread):
    ...
    def __init__(self, ip, row_idx, report_path, pcd_path, get_pcd_path, interval, record_func, pointcloud_header):
        ...
        self.record_func = record_func
        self.interval = interval
        self.capture_pcd_path = os.path.join(
            self.pcd_path, f"{ip.replace('.','_')}.pcd")
    ...
    def run(self):
        ...
        while True:
            ...
            cmd = subprocess.Popen(self.capture_pcd_command, shell=True, stdout=open(
                self.get_pcd_out, "w"), stderr=open(self.get_pcd_err, "a"))
            cmd.wait()
...
class TestMain(QThread):
    ...
    def __init__(self, can_mode, relay_unit, relay_channels, ip_list, record_folder, record_header, times, record_func, record_interval, off_counter, timeout, lidar_mode, pointcloud_func, pointcloud_header):
    ...
    def run_monitor(self, log_path, time_path):
        ...
        for ip_num, ip in enumerate(self.ip_list):
            ...
            record_thread = one_lidar_record_thread(ip, float(
                self.record_interval), self.save_folder, self.record_header, ip_num, self.record_func)
            ...
            raw_save_path = os.path.join(
                log_path, "raw", ip.replace(".", "_"), time_path)
            monitor_thread = MonitorFault(
                ip, log_path, raw_save_path, ip_num, 9600+ip_num, 9100+ip_num, 8600+ip_num, 8100+ip_num, 8010)
            ...
            if os.getenv("pointcloud") == "True":
                pointcloud_thread = PointCloud(ip, ip_num, pointcloud_report_path, pcd_save_folder, self.get_pcd_path, float(
                    self.record_interval), self.pointcloud_func, self.pointcloud_header)
                pointcloud_thread.sigout_pointcloud.connect(
                    self.send_pointcloud_info)
    ...
    def one_cycle(self, power_one_time, i, data_num_power_off, log_path):
        ...
        if sleep_time > 2:
            ...
            for ip in self.ip_list:
                thread = threading.Thread(
                    target=downlog, args=(ip, log_path, time_path,))
                thread.start()
        ...
        kill_client()
        print(
            f"start sleep {int(power_one_time[0]+power_one_time[1]-(time.time()-t))}s")
        if self.cmd_anlyze_log.poll() is not None:
            print("continue analyse log")
            kill_subprocess("log_main.py")
            self.cmd_anlyze_log = subprocess.Popen(
                self.analyse_command, shell=True)
        for i in range(data_num_power_off):
            temp_pow = pow_status
            for row_idx, ip in enumerate(self.ip_list):
                csv_save_path = os.path.join(
                    self.save_folder, 'record_'+ip.replace('.', '_')+'.csv')
                if self.lidar_mode == "CAN":
                    temp = [f" {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}"] + \
                        [-100]*(self.record_header.count(",")-2)+temp_pow
                else:
                    temp = [f" {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}"] + \
                        [-100]*(self.record_header.count(","))
                self.sigout_lidar_info.emit(temp, row_idx)
                if not os.path.exists(csv_save_path):
                    csv_write(csv_save_path, self.record_header)
                csv_write(csv_save_path, temp)
            t0 = (power_one_time[0]+power_one_time[1] -
                  (time.time()-t))/(data_num_power_off-i)
            if t0 > 0:
                time.sleep(t0)
    ...
    def run(self):
        ...
        if os.getenv("current") == "True":
            os.environ["resistor"] = "0.1"
            self.current_monitor = Current_monitor(self.ip_list, self.relay_unit, self.relay_channels, float(
                self.record_interval), self.save_folder, 3)
            self.current_monitor.sigout_plot_data.connect(
                self.send_current_info)
                ...
                try:
                    ...
                    while True:
                        if down_sdk(ip) or down_count > 10:
                            break
                        down_count += 1
                    if os.getenv("full_load") == "True":
                        LidarTool.full_load(ip, True)
        ...
        if self.lidar_mode != "No Power":
            ...
            for time_one in self.times:
                ...
                self.one_cycle(time_one, i, int(
                    self.off_counter), self.save_folder)
                i += 1
        ...
        if self.lidar_mode == "CAN":
            ...
            for ip in self.ip_list:
                if LidarTool.ping(ip, 0.5):
                    thread = threading.Thread(target=downlog, args=(
                        ip, self.save_folder, time_path,))
                    thread.start()
                    threads.append(thread)
            for temp_thread in threads:
                temp_thread.join()
            ...
            cmd = subprocess.Popen(
                self.analyse_command + " -r -mtc 6", shell=True)
            cmd.wait()
            self.sigout_test_finish.emit(self.util_path)
...
class MainCode(QMainWindow, userpage.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        DoubleValidator = QDoubleValidator(
            0, 100000, 3, notation=QtGui.QDoubleValidator.StandardNotation)
        ...
        self.action_lidar_status.triggered.connect(self.stackedWidget_currentChanged(
            0, self.stackedWidget_lidar, self.lb_lidar, "Lidar status"))
        self.action_lidar_pointcloud.triggered.connect(self.stackedWidget_currentChanged(
            1, self.stackedWidget_lidar, self.lb_lidar, "Lidar pointcloud"))
        self.action_Fault.triggered.connect(self.stackedWidget_currentChanged(
            0, self.stackedWidget_current, self.lb_current, "Fault Status"))
        self.action_Current.triggered.connect(self.stackedWidget_currentChanged(
            1, self.stackedWidget_current, self.lb_current, "Current Status(mA)"))
        self.action_Mac_Adress.triggered.connect(self.stackedWidget_currentChanged(
            2, self.stackedWidget_current, self.lb_current, "Update Mac Address"))
        self.action_test_function.triggered.connect(self.stackedWidget_currentChanged(
            3, self.stackedWidget_current, self.lb_current, "Test Function Select"))

        self.action_test_function.trigger()
    ...
    def update_mac_adress(self):
        ...
        ret_mac = re.search(
            "^([A-Fa-f0-9]{2}[:\s]{1}){5}[A-Fa-f0-9]{2}$", mac_adress)
    ...
    def init_plot_widget(self):
        ...
        self.scrollArea_contents_current.setMinimumSize(
            widget_width+30, widget_height*widget_counts)
        for idx, ip in enumerate(self.ip_list):
            qwidget = QtWidgets.QWidget(self.scrollArea_contents_current)
            qwidget.setGeometry(QtCore.QRect(
                10, widget_height*idx, widget_width-30, widget_height))
    ...
    def init_table(self, row_counter, columns_status, columns_pointcloud):
        ...
        for j in range(len(columns_status)):
            self.tbw_data.setHorizontalHeaderItem(
                j, QtWidgets.QTableWidgetItem())
            self.tbw_data.horizontalHeader().setSectionResizeMode(
        ...
        for j in range(len(columns_pointcloud)):
            self.tbw_pointcloud.setHorizontalHeaderItem(
                j, QtWidgets.QTableWidgetItem())
            self.tbw_pointcloud.horizontalHeader().setSectionResizeMode(
                j, QHeaderView.ResizeToContents)
        ...
        self.tbw_fault.horizontalHeader().setSectionResizeMode(
        ...
        for i in range(row_counter):
            ...
            self.tbw_fault.setItem(
                i, 0, QtWidgets.QTableWidgetItem(self.ip_list[i]))
            for j in range(len(columns_status)):
                if j == 0:
                    cell_value = self.ip_list[i]
                else:
                    cell_value = ""
                self.tbw_data.setItem(
                    i, j, QtWidgets.QTableWidgetItem(cell_value))
            for j in range(len(columns_pointcloud)):
                if j == 0:
                    cell_value = self.ip_list[i]
                else:
                    cell_value = ""
                self.tbw_pointcloud.setItem(
                    i, j, QtWidgets.QTableWidgetItem(cell_value))
        ...
        for col_idx in range(self.tbw_fault.columnCount()):
            if self.tbw_fault.horizontalHeaderItem(col_idx).text() == fault:
                if self.tbw_fault.item(row_idx, col_idx) != None:
                    last_fault_count = self.tbw_fault.item(
                        row_idx, col_idx).text()
                    if re.search("\d+", last_fault_count):
                        self.tbw_fault.setItem(row_idx, col_idx, QtWidgets.QTableWidgetItem(
                            f"{int(last_fault_count)+1}"))

                        return
                self.tbw_fault.setItem(
                    row_idx, col_idx, QtWidgets.QTableWidgetItem(f"{1}"))
                return
        last_max_col = self.tbw_fault.columnCount()
        self.tbw_fault.setColumnCount(last_max_col+1)
        self.tbw_fault.setHorizontalHeaderItem(
            last_max_col, QtWidgets.QTableWidgetItem())
        self.tbw_fault.horizontalHeader().setSectionResizeMode(
            last_max_col, QHeaderView.ResizeToContents)
        item = self.tbw_fault.horizontalHeaderItem(last_max_col)
        item.setText(f"{fault}")
        self.tbw_fault.setItem(row_idx, last_max_col,
                               QtWidgets.QTableWidgetItem(f"{1}"))
    ...
    def set_fault(self, fault, row_idx):
        ...
        if fault not in self.scrollArea_list:
            ...
            self.scrollArea_contents_fault.setMinimumSize(
                fault_name_width+19+40*len(self.ip_list), 20+30*len(self.scrollArea_list))
            label = QtWidgets.QLabel(self.scrollArea_contents_fault)
            label.setGeometry(QtCore.QRect(
                10, 30*len(self.scrollArea_list)-10, fault_name_width, 21))
            label.setText(fault)
            setattr(self, f"lb_{fault}", label)
            for idx, ip in enumerate(self.ip_list):
                label = QtWidgets.QLabel(self.scrollArea_contents_fault)
                label.setGeometry(QtCore.QRect(
                    fault_name_width+19+40*idx, 30*len(self.scrollArea_list)-10, 21, 21))
                label.setText("")
                label.setStyleSheet(
        ...
        getattr(self, f"lb_{fault}_{row_idx}").setStyleSheet(
            "border-radius:10px;background-color:rgb(255, 0, 0)")

    @handle_exceptions
    def heal_fault(self, fault, row_idx):
        if fault in self.scrollArea_list:
            getattr(self, f"lb_{fault}_{row_idx}").setStyleSheet(
                "border-radius:10px;background-color:rgb(115, 210, 22)")
    ...
    def init_select_item(self):
        ...
        # may can be json file, but no explain
        for test_name in sorted(os.listdir(self.test_folder)):
    ...
    def power_changed(self):
        print(
            f"current power is {self.cb_power_type.currentText()},please ensure has connect!")
        os.environ["power_type"] = self.cb_power_type.currentText()
        if os.path.exists("power.py"):
            os.remove("power.py")
        shutil.copyfile(os.path.join(
            self.power_folder, f"power_{self.cb_power_type.currentText()}.py"), os.path.join(os.getcwd(), "power.py"))
    ...
    def power_status(self, mode):
        if mode:
            self.lb_power_status.setStyleSheet(
                "border-radius:10px;background-color:rgb(115, 210, 22)")
        else:
            self.lb_power_status.setStyleSheet(
                "border-radius:10px;background-color:rgb(255, 0, 0)")
    ...
    def test_name_changed(self):
        if self.cb_test_name.currentText().strip() != "":
            cfg_file = os.path.join(
                self.test_folder, f"{self.cb_test_name.currentText()}.py")
    ...
    def project_changed(self):
        if self.cb_project.currentText().strip() != "":
            ...
            mata_class = importlib.import_module(
                f"{self.project_folder.strip('.').strip('/')}.{self.cb_project.currentText()}")
    ...
    def read_config(self):
        if hasattr(self, "test_config"):
            ...
            if "relay unit" in self.test_config.keys():
                self.relay_unit = self.test_config["relay unit"]
            else:
                self.relay_unit = 1
            if "relay channel" in self.test_config.keys():
                self.relay_channels = self.test_config["relay channel"]
            else:
                self.relay_channels = range(1, 16)
        ...
        self.scrollArea_contents_fault.setGeometry(
            QtCore.QRect(0, 0, 485, 795))
        self.scrollArea_contents_fault.setObjectName(
    ...
    def test_main(self):
        ...
        test_version = self.lb_version.text()
        test_lidar_mode = self.cb_lidar_mode.currentText()
        test_power_type = self.cb_power_type.currentText()
        test_project = self.cb_project.currentText()
        test_name = self.cb_test_name.currentText()
        test_can_mode = self.cb_can_mode.currentText()
        test_off_counter = self.txt_off_counter.text()
        test_interval = self.txt_record_interval.text()
        test_timeout = self.txt_timeout.text()
        test_relay = self.relay.isChecked()
        test_dsp = self.dsp.isChecked()
        test_pointcloud = self.cb_pointcloud.isChecked()
        test_current = self.cb_current.isChecked()
        test_full_load = self.cb_full_load.isChecked()
        test_info_all = f'''test_version:{test_version}, \
Lidar mode:{test_lidar_mode}, \
Power type:{test_power_type}, \
Project:{test_project}, \
Test name:{test_name}, \
CAN mode:{test_can_mode}, \
Off counter:{test_off_counter}, \
Interval:{test_interval}s, \
Timeout:{test_timeout}s, \
Relay:{test_relay}, \
DSP:{test_dsp}, \
Pointcloud:{test_pointcloud}, \
Current:{test_current}, \
Full load:{test_full_load}'''
        print(test_info_all)
        os.environ["relay"] = f"{test_relay}"
        os.environ["dsp"] = f"{test_dsp}"
        os.environ["pointcloud"] = f"{test_pointcloud}"
        os.environ["current"] = f"{test_current}"
        os.environ["full_load"] = f"{test_full_load}"
        self.test = TestMain(test_can_mode, self.relay_unit, self.relay_channels, self.ip_list, self.save_folder, self.record_header, self.times,
                             self.record_func, test_interval, test_off_counter, test_timeout, test_lidar_mode, self.pointcloud_func, self.pointcloud_header)
    ...
    def test_set_off(self):
        ...
        self.cb_full_load.setEnabled(False)
    ...
    def test_set_on(self):
        ...
        self.cb_full_load.setEnabled(True)
        ...
        for row_idx in range(self.tbw_fault.rowCount()):
            ...
            for col_idx in range(self.tbw_fault.columnCount()):
                if self.tbw_fault.item(row_idx, col_idx) != None:
                    if col_idx != 0:
                        row.append(
                            int(self.tbw_fault.item(row_idx, col_idx).text()))
                    else:
                        row.append(self.tbw_fault.item(
                            row_idx, col_idx).text())
                else:
                    row.append(0)
            df.loc[self.ip_list[row_idx], :] = row
        df.to_excel(os.path.join(self.save_folder,
                    "fault_counter.xlsx"), sheet_name="fault", index=None)
