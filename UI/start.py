...
class TestMain(QThread):
    ...
    def run(self):
        ...
        if os.getenv("current")=="True":
            os.environ["resistor"] = "0.1"
            self.current_monitor = Current_monitor(self.ip_list,self.relay_unit,self.relay_channels,float(self.record_interval),self.save_folder,3)
...
class MainCode(QMainWindow,userpage.Ui_MainWindow):
    ...
    def test_main(self):
        ...
        self.test=TestMain(self.cb_can_mode.currentText(),self.relay_unit,self.relay_channels,self.ip_list,self.save_folder,self.record_header,self.times,self.record_func,self.txt_record_interval.text(),self.txt_off_counter.text(),self.txt_timeout.text(),self.cb_lidar_mode.currentText(),self.pointcloud_func,self.pointcloud_header)
